...
\medskip
\emph{以下为标准示范：为演示新增约束+对偶单纯形法的完整流程，对约束参数作了调整以使原最优解不满足（原题答案为"满足，不变"）。读者应关注方法步骤本身。}

\medskip
\textbf{标准示范——假设不满足新约束：}

引入松弛变量 $x_6 \ge 0$，新约束化为 $2x_1 - x_2 + x_3 + x_6 = 9$。将新行添入最优表，利用 $x_1$ 行和 $x_3$ 行消去新行中基变量的系数。$x_1$ 行系数为 $(1,5,0,3,-1,0\mid 1)$，$x_3$ 行系数为 $(0,-7,1,-5,2,0\mid 2)$。新行 $=$ 新约束系数 $-2\cdot(\text{行}x_1)-\!1\cdot(\text{行}x_3)$，计算得：
...
\[
x^* = \left(\frac{3}{2},\,0,\,1,\,0,\,\frac{1}{2},\,0\right)^T,\quad z^* = \frac{37}{2} = 18.5
\]
