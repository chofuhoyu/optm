...
- **slides-print.pdf** — 课件 A4 打印版（2列×4行=8张/页），考试用
- **slides-reader.pdf** — 课件 Leaf5 阅读器版（2张/页，适配 1680×1264 屏幕）
- **review-reader.pdf** — 复习材料 Leaf5 阅读器版（按知识点编排，2025真题作为例题嵌入）
- **notes-print.pdf** — 考场课堂笔记 A4 打印版（纯知识点，无例题/真题）
...
make slides-print   # 课件 A4 打印版 slides-print.pdf（8张/页）
make slides-reader  # 课件 Leaf5 阅读器版 slides-reader.pdf（2张/页）
make review-reader  # 复习材料 review-reader.pdf（真题答案 + 解读 + 课件）
make notes-print    # 考场课堂笔记 notes-print.pdf（纯知识点，无例题）
...
# === 其他 ===
make clean         # 清理所有 *.aux *.log *.out *.toc（slides/ + review/ + review-old/ + notes/）
```
...
- 编译：`make notes-print`（纳入 `make all`，与其他 PDF 一起编译）
